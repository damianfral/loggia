...
tagWithDirection (t :< b@(Branch b1 b2)) =
  Fix $
    (:< b)
      ( t
          & direction
...
tagWithMargins (attrs :< Branch b1 b2)
  ...
  where
    ...
    go ghostBox b = Fix $ unFix b & tag . margins .~ offsets
      where
...
labelBoxes = (`evalState` 0) . cataM go
  where
    go ::
      TaggedTreeF Attrs Box (TaggedTree Attrs (Int, Box)) ->
...
linkImages imgs = fmap Page . cataM go
  where
    go (a :< Leaf (i, _)) = safeIx (i - 1) imgs <&> Fix . (a :<) . Leaf
...
askValidLayouts imgs =
  ask
    >>= ( \case
      ...
      . filter ((==) (length imgs) . countLayoutLeaves)
...
wastedImageArea scale img box = area imageBox - area box'
  where
    ...
    imageBox =
      box & case compare imgAR boxAR of
        EQ -> id
