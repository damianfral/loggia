...
boxToBoxCenter (x, y) (Box (Segment h1 h2) (Segment v1 v2)) =
  BoxCenter x y l r t d
  where
    l = x - h1
    r = h2 - x
    t = y - v1
...
rotateBoxCenter (BoxCenter x y l r t d) =
  BoxCenter x y t d r l

isInside :: Box -> Box -> Bool
isInside b1 b2 = b1 <> b2 == b1

areOverlapping :: Box -> Box -> Bool
areOverlapping b1 b2 =
  (areSegmentsOverlapped `on` view horizontal) b1 b2
...
calculateOffsets
  ...
  (Box (Segment l2 r2) (Segment t2 b2)) = box
    where
      box = Box (Segment (l2 - l1) (r1 - r2)) (Segment (t2 - t1) (b1 - b2))

-- | Rotate a box 90 degress counter clockwise over a given center.
rotateBox :: (Num a) => (a, a) -> BoxF a -> BoxF a
rotateBox centerPoint =
  boxToBoxCenter centerPoint
    >>> rotateBoxCenter
    >>> boxCenterToBox
