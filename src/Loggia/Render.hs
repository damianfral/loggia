...
imageToCSS (ImageF p _ _ _) = ";background-image: url(" <> p <> ");"

renderPageHtml :: Page -> Html ()
renderPageHtml = generateDivs >>> flip evalState 0
  where
    generateDivs :: Page -> State Int (Html ())
    generateDivs EmptyPage = pure $ div_ [] [ text "asd" ]
    generateDivs (Page page) = case unFix page of
      x :< Leaf a -> do
        i <- modify (+ 1) >> get
        return $ divLeave_ x a (toHtml $ "Box " <> show i)
      x :< Branch f g ->
        divNode_ x
          <$> (mappend <$> generateDivs (Page f) <*> generateDivs (Page g))
    divNode_ attrs =
      div_
        [ style_ $ attrsToCSS attrs,
          class_ "photo-group"
        ]
    divLeave_ attrs img =
      div_
        [ style_ (attrsToCSS attrs <> imageToCSS img),
          class_ "photo-item"
        ]
...
defaultTemplate albumPages = html_ $ do
  ...
  where
    ...
    go (i, (l, r)) = div_ `with` [id_ (pack $ "sheet" <> show i), class_ "album__sheet"] $ do
