...
concatenateHorizontal (Mockup bs1 _ _) (Mockup bs2 _ _) =
  normalizeMockup $
    Mockup (bs1 <> (go <$> bs2)) 2 1
  where
    go (Box h v) = Box (fmap (+ 1) h) v
...
testMockup0 = normalizeMockup $ Mockup [a, b, c] 1 1
  where
...
splitVMockup m x = guard (validSplit split) >> return split
  where
    ...
    split = (m1, m2)
    validSplit (l, r) =
      length (m ^. boxes) == length (l ^. boxes) + length (r ^. boxes)
        && not (null (l ^. boxes))
        && not (null (r ^. boxes))
...
unflatten m = Fix <$> (Branch <$> l <*> r)
  where
    ...
    verticalSplits = mapMaybe (splitVMockup m) (verticalDivisors m)
    horizontalSplits = mapMaybe (splitHMockup m) (horizontalDivisors m)
